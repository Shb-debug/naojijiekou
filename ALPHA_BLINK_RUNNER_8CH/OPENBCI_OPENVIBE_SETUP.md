...
场景依次为：Acquisition client → Channel Selector → 1–30 Hz Temporal Filter → 0.32 秒 epoch / 0.04 秒步进 → Alpha + blink Python Box。8 通道版本的 Channel Selector 默认选择硬件通道 `1;2;3;4;5;6;7;8`，输出顺序为 `Fp1;Fp2;P3;P4;O1;Oz;O2;Pz`。其中前两路用于眨眼检测，后六路用于 Alpha 功率计算。若设备实际电极标签或硬件编号不同，请按实际情况调整选择器，保持两路额前通道在前。
