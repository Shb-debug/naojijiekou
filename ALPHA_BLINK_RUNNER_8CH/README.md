# Alpha Dash 8CH：OpenBCI / OpenViBE 眨眼跳跃游戏
...
算法分两路：Channel Selector 输出的前 2 路短窗时域波形用于眨眼事件，后 6 路的 8–13 Hz 相对功率用于 Alpha 状态遥测。真实设备时只由 OpenViBE Python Box 检测眨眼，`brain_bridge.py` 不读取 LSL、不会运行第二个检测器。眨眼检测采用边缘安全平滑、双通道同步约束、鲁棒自适应基线、候选—确认—恢复状态机和事件冷却，避免单点噪声触发。无硬件演示才使用 `--simulate`。

## 8 通道默认排列

新场景默认选择 `1;2;3;4;5;6;7;8`，并按以下顺序解释：`Fp1;Fp2;P3;P4;O1;Oz;O2;Pz`。前两路必须放置可稳定检测眨眼的额前电极；后六路用于 Alpha 功率。若你的 8 通道电极标签或硬件编号不同，只需在 `SUPER-mario.xml` 的 Channel List 中调整顺序，保持“2 路额前在前、其余 6 路在后”。

## 无硬件演示

```powershell
cd E:\dijikeji\naojijiekou\ALPHA_BLINK_RUNNER_8CH
...
2. 启动 OpenViBE Acquisition Server，连接到 OpenBCI GUI 的 LSL 数据，默认端口 `1024`。
   - LSL 驱动选择 `obci_eeg1`，确认打开的是 8 通道 EEG。
...
真实模式下，`brain_bridge.py` 只等待和接收 OpenViBE 的 HTTP 事件，不读取 LSL，也不会启动备用眨眼检测器。不要再额外启动旧版 LSL 检测脚本。场景默认选择 `1;2;3;4;5;6;7;8`，输出顺序为 `Fp1;Fp2;P3;P4;O1;Oz;O2;Pz`；如果设备标签不同，请调整场景的 Channel Selector，并保持前两路为额前通道。
