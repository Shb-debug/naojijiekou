...
4. 在 OpenViBE Designer 打开 `CARGAME_OpenViBE_SSVEP.xml` 并运行。
...
场景依次为：Acquisition client → Channel Selector → 1–30 Hz Temporal Filter → 0.9 秒 epoch / 0.1 秒步进 → Alpha + blink Python Box。Channel Selector 默认 `1;2;3;4`，脚本按前两路作为 Fp1/Fp2，后两路作为 O1/Oz/O2/Pz 的可用后枕通道。若你的设备通道顺序不同，应在场景中选择实际的前额与后枕通道，并同步修改 `ssvep_config.json` 的名称或索引。
