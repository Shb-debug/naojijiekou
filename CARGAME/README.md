...
算法分两路：前额 Fp1/Fp2 的短窗振幅瞬态用于眨眼事件，后枕 O1/Oz/O2/Pz 的 8–13 Hz 相对功率用于 Alpha 状态遥测。眨眼检测采用滑动鲁棒基线、MAD 自适应阈值、滞回释放和 550 ms refractory，默认每 100 ms 评估一次，避免单点噪声重复触发。无硬件时可用 `--simulate` 或空格/向上箭头演示。
...
打开 <http://127.0.0.1:8000/>，点击右上角连接桥接器，再点击“开始闯关”。真实设备时，在 OpenBCI GUI 的 Networking Widget 开启 LSL EEG 输出；OpenViBE Designer 打开并运行 `CARGAME_OpenViBE_SSVEP.xml`。如果通道标签不是 Fp1/Fp2/O1/Oz/O2/Pz，请在场景的 Channel Selector 和 `ssvep_config.json` 中调整通道。
